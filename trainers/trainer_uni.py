...
class cross_domain_trainer(object):
    ...
    def __init__(self, args):
        ...
        self.dataset_configs.final_out_channels = self.dataset_configs.final_out_channels
    ...
    def train(self):
        ...
        for i in scenarios:
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                # Initialize variables to store best model states
                ...
                # Load best model states before final evaluation
                # feature_extractor.load_state_dict(torch.load(self.fpath))
                # classifier.load_state_dict(torch.load(self.cpath))
                self.algorithm.feature_extractor.load_state_dict(best_feature_extractor_state)
                self.algorithm.classifier.load_state_dict(best_classifier_state)
